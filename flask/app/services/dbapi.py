...
def login(un: str, pw: str):
    ...
    if x.status_code == 200:
        ...
        print(session['token']) # todo remove
        return True
    else:
        return False
...
def get_all_stats():
    x = requests.get(f"{get_db()}/files", verify=False, json={"category": "ui_statistics_collection"})
    if not x.status_code == 200:
        print(x.json())
    return x.json()

def get_stat_collection(collection_id):
    x = requests.get(f"{get_db()}/files/{collection_id}/file", verify=False)
    if not x.status_code == 200:
        print(x.json())
        return None
    return x.json()
...
def get_stat_graphs(graph_list):
    ...
    for graph_data in graph_list:
        x = requests.get(f"{get_db()}/files/{graph_data['file_id']}/file", verify=False)
        if not x.status_code == 200:
            print(x.json())
            continue
        png = Image.open(io.BytesIO(x.content))
        # filename = f"app/static/figures/{graph_data['label']}.png"
...
def post_stat_graphs(graph_list):
    ...
    for path in graph_list:
        ...
        x = requests.post(f"{get_db()}/files", files={
...
def post_stat_obj(name, obj):
    ...
    }, headers=_auth_header(), verify=False)
    if not x.status_code == 200:
        print(x.status_code)
        print(x.json())
        return None
